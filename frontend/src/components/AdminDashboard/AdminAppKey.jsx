...
const AdminAppKey = () => {
  ...
  const fetchData = async () => {
    const { data } = await axios(`http://localhost:5000/keys/${role}`);
    setKeys(data.keys);
  };
  ...
  const deleteApiKey = async (id, name) => {
    await axios.delete(`http://localhost:5000/keys/${id}`, { data: { name } });
  };
                ...
                {keys?.map((key, index) => {
                  return (
                    <tr
                      ...
                      key={index}
                    >
                      <td className='p-3 hover:bg-gray-100'>{key.app_name}</td>
                      ...
                      <td className='p-3 text-red-400 cursor-pointer hover:text-red-600 hover:font-medium'>
                        <button
                          type='button'
                          onClick={() =>
                            deleteApiKey(key.API_KEY, key.app_name)
                          }
                        >
